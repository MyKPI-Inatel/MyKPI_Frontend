...
export const CreateOrganization = () => {
  ...
  async function handleCreation(formData: FormData) {
    ...
    try {
      ...
      swal('Sucesso', 'Organização criada com sucesso', 'success');
    } catch (error) {
      swal('Erro', 'Erro ao criar organização', 'error');
      console.error(error);
    }
  }
      ...
      <DialogContent>
        ...
        >
          <div className="flex flex-col w-full gap-4">
            ...
            <Input type="text" id="id" name="id" placeholder="Identificador da Organização" />
            <Label htmlFor="organization" className="text-left">
              Nome da Organização
            </Label>
            <Input type="text" id="organization" name="name" placeholder="Nome da Organização" />
          </div>
